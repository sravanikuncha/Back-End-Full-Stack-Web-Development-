...
export default class BookRepository {
    ...
    async addReviewToTarget(targetId, target, text, rating) {
        ...
        //create review 
        const reviewData=new reviewModel({text,rating, on_model:target});
        ...
        if(target=='Author'){
            //fin id and update;
            const authorData=await authorsModel.findByIdAndUpdate(filter,updates,{new:true}).populate('reviews');
            if(!authorData){
                 throw new Error("Author not found")
            }
            
        }else if (target=='Book'){
            const bookData=await booksModel.findByIdAndUpdate(filter,updates,{new:true}).populate('reviews');
            if(!bookData){
