...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) {
    ...
    //   { _id: new ObjectId(id) },{ $set: { "tags.$[index]": newTag } },{ arrayFilters: [{ "index": oldTag }] });
  ...
  async deleteTagFromExpense(id, tag) { 
    // const db=getDB();
    // await db.collection(this.collectionName).updateOne({_id:new ObjectId(id)},{$pull:{tags:{$eq:tag}}});
    const db = getDB();
