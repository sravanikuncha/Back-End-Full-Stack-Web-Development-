...
export default class BookRepository {
    ...
    async addReviewToBook(bookId, text, rating) {
        ...
        const reviewForBook=new reviewModel(reviewData);
        await reviewForBook.save();

        //update now in book Model
        const filter={
            _id:bookId
        };
        ...
        const updates={
            $push:{
                reviews:reviewForBook._id
            }
        }

        const updatedBook =await booksModel.updateMany(filter,updates);
